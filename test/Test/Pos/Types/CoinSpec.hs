...
spec = describe "Coin properties" $ do
  ...
  where
    ...
    appliedPortionDownUp = "Applying a coin portion to a coin (up) via\
...
-- NB. these tests should actually fail ! But on most inputs they don't so they are

appliedCoinPortionDown :: (C.CoinPortion, C.Coin) -> Property
appliedCoinPortionDown =
    let applyViaDouble (C.getCoinPortion -> p, C.unsafeGetCoin -> c) =
            C.mkCoin . floor $
                (realToFrac p / realToFrac C.coinPortionDenominator) *
                (realToFrac c :: Double)
    in uncurry C.applyCoinPortion .=. applyViaDouble
